<#
    .SYNOPSIS
        Unit test for DSC_DnsRecordTxt DSC resource.
#>
...
Describe DnsRecordTxt -Tag 'DnsRecord', 'DnsRecordTxt' {
    Context 'Constructors' {
        It 'Should not throw an exception when instantiated' {
            InModuleScope -ScriptBlock {
                Set-StrictMode -Version 1.0

                { [DnsRecordTxt]::new() } | Should -Not -Throw
            }
        }
    ...
    Context 'Type creation' {
        It 'Should be type named DnsRecordTxt' {
            InModuleScope -ScriptBlock {
                Set-StrictMode -Version 1.0

                $instance = [DnsRecordTxt]::new()
                $instance.GetType().Name | Should -Be 'DnsRecordTxt'
            }
        }
    }
}

Describe 'Testing DnsRecordTxt Get Method' -Tag 'Get', 'DnsRecord', 'DnsRecordTxt' {
    BeforeEach {
        InModuleScope -ScriptBlock {
            Set-StrictMode -Version 1.0

            $script:instanceDesiredState = [DnsRecordTxt] @{
...
Describe 'Testing DnsRecordTxt Test Method' -Tag 'Test', 'DnsRecord', 'DnsRecordTxt' {
            ...
            BeforeEach {
                InModuleScope -ScriptBlock {
                    ...
                    $script:instanceDesiredState | Add-Member -Force -MemberType ScriptMethod -Name Get -Value {
                        $mockInstanceCurrentState = [DnsRecordTxt] @{
                            ZoneName        = 'contoso.com'
                            Name            = 'test'
                            DescriptiveText = 'Example text for test.contoso.com TXT record.'
                            Ensure          = [Ensure]::Absent
...
Describe 'Testing DnsRecordTxt Set Method' -Tag 'Set', 'DnsRecord', 'DnsRecordTxt' {
    BeforeAll {
